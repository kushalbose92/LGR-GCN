...
class GAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, latent_dim, with_latent, num_layers=2,
        ...
        if with_latent == 'True':
            self.convs.append(
                GATConv(in_channels+latent_dim, hidden_channels, heads=heads, concat=True, add_self_loops=add_self_loops))
        else:
            self.convs.append(
    ...
    def forward(self, x, edge_index, adjs=None, x_batch=None):
...
class LINKX(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, latent_dim, with_latent, num_layers, num_nodes, dropout=.5, cache=False, inner_activation=False, inner_dropout=False, init_layers_A=1, init_layers_X=1):
        super(LINKX, self).__init__()
        self.mlpA = MLP(num_nodes, hidden_channels,
                        hidden_channels, init_layers_A, dropout=0)
        
        if with_latent == 'True':
            self.mlpX = MLP(in_channels+latent_dim, hidden_channels,
